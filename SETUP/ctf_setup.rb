...
def read_ssh_private_key
  ...
  unless File.readable?(private_key_path)
    raise "Cannot read private key file at #{private_key_path}. Ensure it is readable by the current user (e.g., adjust permissions with 'sudo chmod 640 /tmp/git_key' or run as root)."
  end
...
user2 = User.new(
  ...
  email: 'x-registry-bot@secdojo.com',
  name: 'Registry Bot',
...
begin
  ...
  public_repo.repository.create_file(
    ...
    <<~YAML,
      ...
      variables:
        CI_REGISTRY: "registry.secdojo.com"
        API_ENDPOINT: "https://api.secdojo.com/v1"
